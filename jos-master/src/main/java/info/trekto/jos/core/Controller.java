...
Código fuente :
    ...
    CheckAllCollisionsRunDouble, CheckAllCollisionsRunFloat.
Grau Informàtica
...
public enum Controller {
    ...
    public static final int CPU_DEFAULT_THRESHOLD = 0;
    ...
    public static void main(String[] args) {
        ...
        if (args[0].compareToIgnoreCase("-t") == 0) {
            C.MThreads = Integer.parseInt(args[1]);
            System.out.println("Threads: " + C.MThreads);
        }
    ...
    static Simulation createSimulation(SimulationProperties properties, ExecutionMode executionMode) {
        ...
        if (numberType == ARBITRARY_PRECISION || (executionMode == AUTO && n <= cpuThreshold)) {
            return new SimulationAP(properties);
        } else if (executionMode == CPU) {
